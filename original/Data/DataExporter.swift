...
enum DataExporter {
    ...
    static func export(players: [Player], games: [Game]) -> Data? {
    ...
    static func importData(_ data: Data) -> (players: [Player], games: [Game])? {
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        guard let exported = try? decoder.decode(ExportData.self, from: data) else { return nil }

        let players = exported.players.map { dto -> Player in
            let role = PlayerRole(rawValue: dto.role) ?? .player
            var p = Player(name: dto.name, role: role, goals: dto.goals, assists: dto.assists, bonusPoints: dto.bonusPoints)
        ...
        let games = exported.games.map { dto -> Game in
            Game(
    ...
    static func saveToDisk(players: [Player], games: [Game]) -> Bool {
    ...
    static func loadFromDisk() -> (players: [Player], games: [Game])? {

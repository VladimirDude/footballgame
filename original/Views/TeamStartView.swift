...
struct TeamStartView: View {
    ...
    var body: some View {
        ...
        .navigationTitle("Cognaize Futsal")
        .toolbarColorScheme(.dark, for: .navigationBar)
        .navigationDestination(isPresented: $showProfile) { ProfileView(vm: vm) }
        .navigationDestination(isPresented: $show3DPreview) { CharacterViewerScreen() }
        .navigationDestination(isPresented: $showCoachDetail) { CoachDetailView(coaches: vm.coaches) }
        .navigationDestination(isPresented: $showGames) { GamesView(vm: vm) }
    ...
    private var adminBadge: some View {
        Menu {
            ...
            HStack(spacing: 4) {
                ...
                Text(AppConfig.currentRole.rawValue)
                    .font(.system(size: 11, weight: .bold))
            }
            .padding(.horizontal, 10)
            .padding(.vertical, 5)
            .background(vm.isAdmin ? Theme.red.opacity(0.2) : Theme.blue.opacity(0.15), in: Capsule())
            .foregroundStyle(vm.isAdmin ? Theme.red : Theme.blue)
        }
    }

    // MARK: - Header with board.png background
            ...
            Image("board")
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 20)
                        .fill(
                            LinearGradient(colors: [Theme.bg.opacity(0.3), Theme.bg.opacity(0.85)],
                    ...
                    if vm.isAdmin {
                        Button { showTeamIconPicker = true } label: {
                            Image(systemName: "camera.circle.fill")
                                ...
                                .background(Circle().fill(Theme.blue).frame(width: 22, height: 22))
                        }
                        .offset(x: 20, y: 20)
                    }
                }
    ...
    private var overviewCards: some View {
        VStack(spacing: 10) {
            ...
            HStack(spacing: 10) {
                StatCard(title: "Players", value: "\(vm.playerCount)", icon: "figure.run", tint: Theme.blue)
                StatCard(title: "Goalkeepers", value: "\(vm.goalkeeperCount)", icon: "hand.raised.fill", tint: Theme.orange)
                Button { showCoachDetail = true } label: {
                    StatCard(title: "Coaches", value: "\(vm.coachCount)", icon: "person.badge.clock.fill", tint: Theme.purple)
                }.buttonStyle(.plain)
            }
        }
    }
            ...
            HStack {
                ...
                if vm.isAdmin {
                    ...
                    Button { showAddGame = true } label: {
                        Image(systemName: "plus.circle.fill").font(.system(size: 20)).foregroundStyle(Theme.green)
                    }
                }
            }
    ...
    private func recentGameRow(_ game: Game) -> some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text("vs \(game.opponent)").font(.system(size: 14, weight: .semibold)).foregroundStyle(Theme.textPrimary).lineLimit(1)
                Text(game.scorers.prefix(3).joined(separator: ", ")).font(.system(size: 11, weight: .medium)).foregroundStyle(Theme.textTertiary).lineLimit(1)
            }
    ...
    private func gameColor(_ r: GameResult) -> Color {
    ...
    private func goalkeeperCard(_ gk: Player) -> some View {
        HStack(spacing: 14) {
            PlayerAvatarView(image: gk.photo, name: gk.name, role: .goalkeeper, size: 44)

            VStack(alignment: .leading, spacing: 3) {
                Text(gk.name).font(.system(size: 16, weight: .bold)).foregroundStyle(Theme.textPrimary)
                if let s = gk.goalkeeperStats {
                    HStack(spacing: 12) {
                        Label("\(s.matchesAttended)", systemImage: "checkmark.circle")
                        Label("\(s.goalsConceded)", systemImage: "xmark.circle").foregroundStyle(Theme.red.opacity(0.8))
                        Label("\(s.cleanSheets)", systemImage: "shield.checkered").foregroundStyle(Theme.green)
            ...
            if let s = gk.goalkeeperStats {
                VStack(alignment: .trailing, spacing: 2) {
                    Text(String(format: "%.1f", s.rating)).font(.system(size: 20, weight: .bold, design: .rounded)).foregroundStyle(Theme.orange)
                    Text("rating").font(.system(size: 9, weight: .semibold)).foregroundStyle(Theme.textTertiary).textCase(.uppercase)
                }
            }
        }
        .padding(.horizontal, 16).padding(.vertical, 12)
        .background(Theme.cardBg, in: RoundedRectangle(cornerRadius: 14))
        .overlay(RoundedRectangle(cornerRadius: 14).stroke(Theme.orange.opacity(0.15), lineWidth: 1))
    }
        ...
        VStack(spacing: 10) {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 8) {
                        ForEach(FilterOption.allCases) { o in
                            chipButton(label: o.rawValue, isSelected: vm.filterOption == o, tint: Theme.blue) { vm.filterOption = o }
                        }
                    }
                }
                HStack(spacing: 8) {
                    Text("Sort:").font(.system(size: 12, weight: .medium)).foregroundStyle(Theme.textTertiary)
                    ForEach(SortOption.allCases) { o in
                        chipButton(label: o.rawValue, isSelected: vm.sortOption == o, tint: Theme.purple) { vm.sortOption = o }
                    }
                    Spacer()
                }
            }
            ...
            VStack(spacing: 0) {
                ForEach(Array(vm.filteredPlayers.enumerated()), id: \.element.id) { i, p in
                    ...
                    if i < vm.filteredPlayers.count - 1 { Divider().overlay(Theme.cardBorder).padding(.horizontal, 12) }
                }
            }
            .background(Theme.cardBg, in: RoundedRectangle(cornerRadius: 14))
    ...
    private var tableHeader: some View {
        ...
        .font(.system(size: 10, weight: .bold)).foregroundStyle(Theme.textTertiary).textCase(.uppercase).tracking(0.3)
    ...
    private var actionButtons: some View {
        VStack(spacing: 10) {
            Button { show3DPreview = true } label: {
                HStack(spacing: 10) {
                    Image(systemName: "cube.fill").font(.system(size: 18))
                    Text("Open 3D Preview").font(.system(size: 16, weight: .bold))
                }
                .frame(maxWidth: .infinity).padding(.vertical, 15)
                .foregroundStyle(.white)
                .background(Theme.blueGradient, in: RoundedRectangle(cornerRadius: 14))
            }
            ...
            Button { showCoachDetail = true } label: {
                ...
                .background(Theme.purple.opacity(0.12), in: RoundedRectangle(cornerRadius: 14))
                .overlay(RoundedRectangle(cornerRadius: 14).stroke(Theme.purple.opacity(0.2), lineWidth: 1))
            }
        }
    }
    ...
    private func chipButton(label: String, isSelected: Bool, tint: Color, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(label)
                ...
                .background(isSelected ? AnyShapeStyle(tint.gradient) : AnyShapeStyle(Theme.surface), in: Capsule())
                .overlay(Capsule().stroke(isSelected ? Color.clear : Theme.cardBorder, lineWidth: 1))
        }.buttonStyle(.plain)
    }
}

...
struct EditPlayerSheet: View {
    ...
    private var player: Player? { vm.players.first { $0.id == playerId } }

    init(vm: TeamViewModel, playerId: UUID) {
        self.vm = vm
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Add") {
                        ...
                        var p = Player(name: name, role: role, photo: photo)
...
struct EditGameSheet: View {
    ...
    init(vm: TeamViewModel, game: Game? = nil) {
        self.vm = vm
        let g = game ?? Game()

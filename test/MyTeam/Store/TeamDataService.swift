...
protocol TeamDataServiceProtocol {
    func fetchPlayers() -> [TeamPlayer]
    func fetchGames() -> [TeamGame]
}

struct TeamDataService: TeamDataServiceProtocol {

    func fetchPlayers() -> [TeamPlayer] { Self.cognaizeRoster }
    func fetchGames() -> [TeamGame] { Self.cognaizeGames }

    // MARK: - Roster

    static let cognaizeRoster: [TeamPlayer] = {
        var r: [TeamPlayer] = []
        r.append(TeamPlayer(name: "Narek",  role: .player, goals: 9, assists: 3))
        r.append(TeamPlayer(name: "Rob",    role: .player, goals: 7, assists: 5, bonusPoints: 2.5))
        r.append(TeamPlayer(name: "Aro",    role: .player, goals: 6, assists: 6, bonusPoints: 1.0))
        r.append(TeamPlayer(name: "Garo",   role: .player, goals: 5, assists: 4, bonusPoints: 0.5))
        r.append(TeamPlayer(name: "Gagik",  role: .player, goals: 4, assists: 3, bonusPoints: 2.0))
        r.append(TeamPlayer(name: "Rafo",   role: .player, goals: 4, assists: 2, bonusPoints: 0.5))
        r.append(TeamPlayer(name: "Armen",  role: .player, goals: 3, assists: 5, bonusPoints: 1.0))
        r.append(TeamPlayer(name: "Galust", role: .player, goals: 3, assists: 3, bonusPoints: 0.5))
        r.append(TeamPlayer(name: "Vazgen", role: .player, goals: 3, assists: 2))
        r.append(TeamPlayer(name: "Hayko",  role: .player, goals: 2, assists: 4, bonusPoints: 0.5))
        r.append(TeamPlayer(name: "Seyran", role: .player, goals: 2, assists: 3))
        r.append(TeamPlayer(name: "Hamo",   role: .player, goals: 2, assists: 2, bonusPoints: 0.5))
        r.append(TeamPlayer(name: "Edo",    role: .player, goals: 1, assists: 3))
        r.append(TeamPlayer(name: "David",  role: .player, goals: 1, assists: 2, bonusPoints: 0.5))
        r.append(TeamPlayer(name: "Gugo",   role: .player, goals: 2, assists: 1, bonusPoints: 2.0))
        r.append(TeamPlayer(name: "Grigor", role: .player, goals: 1, assists: 2, bonusPoints: 2.0))

        r.append(TeamPlayer(name: "Erik",  role: .goalkeeper, bonusPoints: 2.0,
                         goalkeeperStats: GoalkeeperStats(matchesAttended: 8, goalsConceded: 12, cleanSheets: 3)))
        r.append(TeamPlayer(name: "Artem", role: .goalkeeper, assists: 1, bonusPoints: 2.0,
                         goalkeeperStats: GoalkeeperStats(matchesAttended: 6, goalsConceded: 9, cleanSheets: 2)))

        r.append(TeamPlayer(name: "Hakob", role: .coach,
        ...
        r.append(TeamPlayer(name: "Vahag", role: .coach,
    ...
    static let cognaizeGames: [TeamGame] = [
        TeamGame(date: date(2025,4,12), opponent: "Mantashyants", goalsFor: 9, goalsAgainst: 0,
        ...
        TeamGame(date: date(2025,4,5), opponent: "Mars", goalsFor: 3, goalsAgainst: 2,
        ...
        TeamGame(date: date(2025,3,29), opponent: "Real-Synopsis", goalsFor: 2, goalsAgainst: 1,
             scorers: ["Aro","Garo"],
             goalDetails: [
